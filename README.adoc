...
----
.. Create services and activate patches (required for Cemu, Azahar and Dolphin):
+
[source,console]
----
...
The configuration generators for Cemu, Azahar and Dolphin in current batocera don't offer support for motion and touch control (besides DS4 support in Dolphin).
...
Each time a motion service is started or stopped, the batocera configuration is updated so that Cemu, Azahar and Dolphin will read
...
|USB Dongle
|https://github.com/C0rn3j/sc-controller[sc-controller]
| 26768
...
To use a motion provider with an emulator other than Cemu, Azahar or Dolphin, configure the emulator to listen to `localhost` and the provider's UDP port. The https://github.com/joaorb64/joycond-cemuhook/wiki[joycond-cemuhook wiki] has detailed explanations for popular emulators.
...
wii.cemuhook_server_ip=<provider-host>
wii.cemuhook_server_port=<provider-port>
----
...
### Games
#### multi_bomberman
_MULTI_BOMBERMAN_ integrates https://github.com/alexis-puska/lr-multi-bomberman[alexis-puska/lr-multi-bomberman],
a Bomberman game for up to 16 players. Once enabled, it will be found within _Ports_.
...
Installing archlinux package patch in revision 2024-12-21
...
patching file /usr/lib/python3.12/site-packages/configgen/generators/dolphin/dolphinControllers.py
Dolphin generator was patched for cemuhook.
----
+
...
wii.cemuhook_server_ip=localhost
wii.cemuhook_server_port=26766 // <1>
----
<1> Motion Port
